document.addEventListener('DOMContentLoaded', () => {
    ...
    // Helper untuk auto download URL TikWM
    ...
    // 6. Search Presets via TikWM Feed Search API
    ...
    async function loadPresets(keywords) {
        ...
        try {
            ...
            const res = await fetch(`${TIKWM_BASE}/api/feed/search`, {
                method: 'POST',
                body: formData
            });
            ...
            if (json.code === 0 && json.data && json.data.videos && json.data.videos.length > 0) {
                ...
                json.data.videos.slice(0, 6).forEach(video => {
                    const coverUrl = video.cover?.startsWith('http') ? video.cover : `${TIKWM_BASE}${video.cover}`;
                    const likes = video.digg_count ? (video.digg_count / 1000).toFixed(1) + 'K' : '0';
                    const videoTitle = video.title || 'Preset TikTok';

                    const card = document.createElement('div');
                    card.className = 'preset-card';
                    card.innerHTML = `
                        <img src="${coverUrl}" alt="Preset Cover" onerror="this.src='https://via.placeholder.com/300x400'">
                    ...
                    card.onclick = () => {
                        const videoLink = video.play ? `${TIKWM_BASE}${video.play}` : `https://www.tiktok.com/@${video.author?.unique_id}/video/${video.video_id}`;
            ...
            console.error(err);
            presetGrid.innerHTML = '<p style="grid-column: span 2; text-align: center; font-size: 0.8rem; color: #ff6b6b;">Gagal memuat preset.</p>';
        }
    }
    ...
    // Inisialisasi awal pencarian preset
    loadPresets('preset');
});
